...
function MenubarTrigger({ className, ...props }: React.ComponentProps<typeof DropdownMenuTrigger>) {
  return (
    <DropdownMenuTrigger
      ...
      className={cn(
        'hover:bg-muted aria-expanded:bg-muted outline-hidden flex select-none items-center rounded-sm px-2 py-1 text-sm font-medium',
        className,
      )}
      {...props}
    />
...
function MenubarItem({
  ...
  return (
    <DropdownMenuItem
      ...
      className={cn(
        "focus:bg-accent focus:text-accent-foreground data-[variant=destructive]:text-destructive data-[variant=destructive]:focus:bg-destructive/10 dark:data-[variant=destructive]:focus:bg-destructive/20 data-[variant=destructive]:focus:text-destructive data-[variant=destructive]:*:[svg]:!text-destructive not-data-[variant=destructive]:focus:**:text-accent-foreground group/menubar-item gap-2 rounded-sm px-2 py-1.5 text-sm data-[inset]:pl-8 data-[disabled]:opacity-50 [&_svg:not([class*='size-'])]:size-4",
        className,
      )}
      {...props}
    />
...
function MenubarCheckboxItem({
  ...
  return (
    <MenuPrimitive.CheckboxItem
      ...
      className={cn(
        'focus:bg-accent focus:text-accent-foreground focus:**:text-accent-foreground data-disabled:opacity-50 outline-hidden data-disabled:pointer-events-none data-disabled:opacity-50 relative flex cursor-default select-none items-center gap-2 rounded-md py-1.5 pl-8 pr-2 text-sm [&_svg]:pointer-events-none [&_svg]:shrink-0',
        className,
      )}
      checked={checked}
      {...props}
...
function MenubarRadioItem({ className, children, ...props }: MenuPrimitive.RadioItem.Props) {
  return (
    <MenuPrimitive.RadioItem
      ...
      className={cn(
        "focus:bg-accent focus:text-accent-foreground focus:**:text-accent-foreground data-disabled:opacity-50 outline-hidden data-disabled:pointer-events-none relative flex cursor-default select-none items-center gap-2 rounded-md py-1.5 pl-8 pr-2 text-sm [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
        className,
      )}
      {...props}
    >
