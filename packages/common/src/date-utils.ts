...
export const format = (date: string, type: keyof typeof DATE_FORMATS = 'D_MMM_YYYY') => {
  const formatString = DATE_FORMATS[type];
  return dayjs(date).format(formatString);
};

export const formatLocalTime = (date: string, type: keyof typeof DATE_FORMATS = 'D_MMM_YYYY') => {
  const formatString = DATE_FORMATS[type];
